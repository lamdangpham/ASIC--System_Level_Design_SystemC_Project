...
public:
	...
	void SCI_setting(string, string, string, string, string, string, string, string); // for asynchronous
	void SCI_setting(string, string, string, string, string, string); // for synchronous
	void SCI_BitRate(int);
	void SCI_WBuffTrans(sc_bv<8>);
	void SCI_WBuffRecei(sc_bv<8>);
	void SCI_enTrans();
	void SCI_disTrans();
	sc_bv<12> SCI_ProcDatAsynTrans(sc_bv<8>, sc_bv<12> &);
	int SCI_check_num1(sc_bv<8>);
	sc_bv<8> SCI_ProcDatAsynRecei(sc_bv<12>, sc_bv<8> &);
};