...
SCI::SCI(sc_module_name name, sc_time period_): SCKi("SCKi"), RXDi("RXDi"), TXDi("TXDi"), SCI_Func(name), i_socketSCI_to_CPU("i_socketSCI_to_CPU"), t_socketSCI("t_socketSCI"), period(period_)
{
	data_trans_external = "000000000000";
	data_recei_external = "00000000";
	data_recei_CPU = "00000000";

	SCI_setting("asynchronous", "external_clock", "8-bit", "parity", "even", "two_stopbit", "LSB");
	//SCI_setting("synchronous", "external_clock", "MSB");
	SCI_BitRate(32);
...
void SCI::tlm_write_0()
{
	...
	i_socketSCI_to_CPU->b_transport( *trans, delay );  // Blocking transport call
...
void SCI::b_transport( tlm::tlm_generic_payload& trans, sc_time& delay )
{
	...
	cout << "\t\t\t\t\t\t\t\t\t\t\tData CPU " << name() << " send to SCI " << name() << " " << data_recei_CPU << " " << endl; //sc_time_stamp() << endl;
